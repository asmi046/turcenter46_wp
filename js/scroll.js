...
// Функция для скролла блока filters-sec в Туры на море
function fix_block(scr_fix_block, scr_value) {
	...
	for (let index = 0; index < scr_fix_block.length; index++) {
		...
		const item = block.querySelector('.filters-sec_side-block');
		if (!block_width) { block_width = 0; }
		if (window_width > block_width) {
			if (item.offsetHeight < window_height - (header_height + 30)) {
				if (scr_value > offset(block).top - (header_height + 15)) {
					item.style.cssText = "position:fixed;bottom:auto;z-index:50000000;background:#fff;box-shadow: 0px 0px 5px 0px rgb(0 0 0 / 75%);top:0px;width:" + block.offsetWidth + "px;left:" + offset(block).left + "px;";
				} else {
					gotoRelative(item);
				}
